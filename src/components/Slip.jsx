...
export const Slip = React.forwardRef(({ data }, ref) => {
    ...
    return (
        <div ref={ref} className="w-[25.5cm] h-[7.2cm] bg-white text-black flex text-[9px] leading-tight font-sans overflow-hidden box-border p-0 print:border-none print:w-[25.5cm] print:h-[7.2cm] print:overflow-hidden dark:shadow-[0_0_20px_rgba(0,100,255,0.15)] transition-shadow">
            {/* Left Metadata Section - Rotated Text */}
            <div className="w-[2.2cm] h-full relative border-r border-gray-400 flex-shrink-0 bg-gray-50/50 self-stretch">
                <div
                ...
                >
                    ...
                    <span className="flex gap-2"><span>Name: {data.name}</span> <span>Design No: {data.designNo}</span></span>
                    <span>Serial Count: {data.serialCount}</span>
                </div>
            </div>

            {/* Diagram Section */}
            <div className="w-[7cm] h-full flex flex-col items-center justify-center border-r border-gray-400 p-2 flex-shrink-0">
                    ...
                    <table key={chunkIdx} className="flex-1 border-collapse border border-gray-400 h-full" style={{ fontSize: `${fontSizePx}px` }}>
                        <thead>
                            <tr className="bg-gray-200 text-center font-bold" style={{ height: `${rowHeightPx}px` }}>
                                <th className="border border-gray-400 px-0.5 w-[15%]">ID</th>
                                <th className="border border-gray-400 px-0.5 w-[45%]">Range</th>
